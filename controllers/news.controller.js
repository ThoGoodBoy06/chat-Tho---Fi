...
const getNewsContent = async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      } else if (newsItem.link.includes("techcrunch.com")) {
        // TechCrunch sử dụng class "entry-content"
        const contentMatch = htmlText.match(/<div[^>]*class="[^"]*entry-content[^"]*"[^>]*>([\s\S]*?)<\/div>\s*<\/div>/);
        if (contentMatch) {
          fullContent = contentMatch[1];
        } else {
          // Fallback tìm kiếm thẻ article chung
