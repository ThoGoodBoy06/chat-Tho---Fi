...
async function sendNativeNotification(msg) {
            ...
            if (permStatus.display === "granted") {
                await LocalNotifications.schedule({
                    notifications: [{
                        ...
                        extra: { conversationId: msg.conversationId, senderId: msg.senderId, senderName: senderName, senderAvatar: sender.avatar || "" }
                    }]
                });
            }
        } catch (error) {
    ...
    // 2. DÀNH CHO TRÌNH DUYỆT WEB (CHROME, SAFARI, EDGE TRÊN MÁY TÍNH & ĐIỆN THOẠI)
        ...
        const notification = new Notification(senderName, { body: snippet, icon: avatarUrl });
