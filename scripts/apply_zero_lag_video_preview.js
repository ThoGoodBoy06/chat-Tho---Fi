...
console.log('🚀 Đang áp dụng Zero-Lag Video Preview (BoxDecoration, 0% Stack, 100% không bao giờ lag/treo)...');
...
jsFiles.forEach(jf => {
  ...
  // Tìm và thay thế khối video cũ trong aaY
  const vidRegex = /var _isVid=\(o==="video"\)[\s\S]*?new A\.aVidTap\(_vUrl\)[\s\S]*?B\.ao\);[\r\n\s]*\}/;

  const zeroLagVidBlock = `var _isVid=(o==="video")||(a.videoUrl!=null&&a.videoUrl.length>0)||(s&&typeof s==="string"&&(s.indexOf(".mp4")!==-1||s.indexOf(".mov")!==-1||s.indexOf(".webm")!==-1||s.indexOf("/uploads/video_")!==-1||s.indexOf("/videos/")!==-1));
if(_isVid){
...
var _boxDec=new A.ak(new A.q(4279311146),_decImg,d,A.ag(12),d,d,B.t);
...
return A.dr(d,_row,B.M,!1,d,d,d,d,d,d,d,d,d,d,d,d,d,d,d,d,d,d,d,d,d,new A.aVidTap(_vUrl),d,d,d,d,d,d,!1,B.ao);
  ...
  if (vidRegex.test(js)) {
    js = js.replace(vidRegex, zeroLagVidBlock);
    fs.writeFileSync(jf, js, 'utf8');
    console.log(`✅ [${jf}] Đã cập nhật thành Zero-Lag Video Preview!`);
  } else {
    console.error(`❌ [${jf}] Không tìm thấy vidRegex!`);
  }
});
