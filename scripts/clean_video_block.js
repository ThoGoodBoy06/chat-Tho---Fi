...
console.log('🔧 Sửa sạch sẽ khối Video Block trong main.dart.js...');
...
const cleanBlock = `var _isVid=(o==="video")||(a.videoUrl!=null&&a.videoUrl.length>0)||(s&&typeof s==="string"&&(s.indexOf(".mp4")!==-1||s.indexOf(".mov")!==-1||s.indexOf(".webm")!==-1||s.indexOf("/uploads/video_")!==-1||s.indexOf("/videos/")!==-1));
if(_isVid){
...
jsFiles.forEach(jf => {
  ...
  if (start !== -1 && end !== -1) {
    content = content.slice(0, start) + cleanBlock + content.slice(end);
    fs.writeFileSync(jf, content, 'utf8');
    console.log(`✅ [${jf}] Đã thay thế chuẩn xác (start=${start}, end=${end})`);
  } else {
