...
async function run() {
  ...
  try {
    ...
    // Cuộn lên một chút để xem tin nhắn album (ở khoảng giữa lịch sử gần đây)
    for (let i = 0; i < 5; i++) {
      await send('Input.dispatchMouseEvent', { type: 'mouseWheel', x: 250, y: 500, deltaX: 0, deltaY: -600 });
      await new Promise(r => setTimeout(r, 200));
    }
    await new Promise(r => setTimeout(r, 2000));

    // Chụp ảnh màn hình chat để xem Chồng Thẻ Ảnh 3D hiển thị thực tế
    const shot = await send('Page.captureScreenshot');
    fs.writeFileSync('scripts/verified_3d_photo_deck_chat.png', Buffer.from(shot.data, 'base64'));
    console.log('✅ Đã lưu scripts/verified_3d_photo_deck_chat.png');
