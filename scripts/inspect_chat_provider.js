...
async function run() {
  ...
  try {
    ...
    // Click vào item cuộc trò chuyện Quản trị viên (y: 300)
    ...
    // Kiểm tra window._activeChatProvider và messages
    const evalRes = await send('Runtime.evaluate', {
      expression: `
        (() => {
          const p = window._activeChatProvider;
          const s = window._activeChatScreenState;
          if (!p) return { error: 'No activeChatProvider' };
          const msgs = p.d || [];
          return {
            hasProvider: true,
            hasScreenState: !!s,
            convId: p.c ? p.c.a : null,
            userId: p.a ? p.a.a : null,
    ...
    console.log('Provider state:', JSON.stringify(evalRes.result.value, null, 2));
