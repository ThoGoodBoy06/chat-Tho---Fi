...
async function main() {
  ...
  const chrome = spawn(chromePath, [
    ...
    `--user-data-dir=C:\\temp\\chrome_flow_${Date.now()}`,
    'http://localhost:3000'
  ]);
  ...
  try {
    ...
    // Click on '+' button at (430, 620)
    console.log('Clicking on + button at (430, 620)...');
    await send('Input.dispatchMouseEvent', { type: 'mousePressed', x: 430, y: 620, button: 'left', clickCount: 1 });
    await send('Input.dispatchMouseEvent', { type: 'mouseReleased', x: 430, y: 620, button: 'left', clickCount: 1 });

    await new Promise(r => setTimeout(r, 2000));

    const menuShot = await send('Page.captureScreenshot');
    fs.writeFileSync('scripts/attachment_menu.png', Buffer.from(menuShot.data, 'base64'));
    console.log('Screenshot saved to scripts/attachment_menu.png');
